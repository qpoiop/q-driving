...
export class Car {
    ...
    public update() {
        ...
        const isKeyLeft = this.input.isKeyPressed("a") || this.input.isKeyPressed("arrowleft")
        const isKeyRight = this.input.isKeyPressed("d") || this.input.isKeyPressed("arrowright")
        const isKeyUp = this.input.isKeyPressed("w") || this.input.isKeyPressed("arrowup")
        const isKeyDown = this.input.isKeyPressed("s") || this.input.isKeyPressed("arrowdown")

        const steerInput = inputX || (isKeyLeft ? 1 : isKeyRight ? -1 : 0)
        const accelInput = inputY || (isKeyUp ? -1 : isKeyDown ? 1 : 0)

        this.steeringAngle += steerInput * this.steeringAccel
        ...
        if (accelInput < -0.2) this.acceleration += this.accelerationRate
        else if (accelInput > 0.2) this.acceleration -= this.brakeRate
        ...
        const terrainY = this.world.getHeightAt(nextPos)
